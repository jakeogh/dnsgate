...
def dnsgate(config, mode, block_at_psl, restart_dnsmasq, output, backup, noclobber,
    ...
    config_dict = {
        ...
        "disable": disable,
        "enable": enable,
        "dnsmasq_config": dnsmasq_config.name
        }
    ...
    if install_help:
        if mode == 'dnsmasq':
            dnsmasq_install_help(output.name, dnsmasq_config.name)
        elif mode == 'hosts':
            hosts_install_help(output.name)
        quit(0)

    if disable and mode == 'dnsmasq':
    ...
    eprint("Writing output file: %s in %s format", output.name, mode, level=LOG_LEVELS['INFO'])
