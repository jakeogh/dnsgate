...
def append_to_local_rule_file(rule_file, idn):
    ceprint("attempting to append %s to %s", idn, rule_file, level=LOG['INFO'])
    hostname = idn.encode('idna').decode('ascii')
    ceprint("appending hostname: %s to %s", hostname, rule_file, level=LOG['DEBUG'])
    line = hostname + '\n'
    ...
    if 'dnsgate configure' not in ' '.join(sys.argv):
        if 'dnsgate.py configure' not in ' '.join(sys.argv):
            try:
                ...
                ceprint("No configuration file found, run " +
            ...
            try:
                ...
                ceprint('ERROR: ' + CONFIG_FILE + ' has no "output" defined. ' +
                       "run 'dnsgate configure --help' to fix. Exiting.",
                       level=LOG['ERROR'])
                quit(1)
            assert isinstance(output_path, str)
            if not os.path.exists(os.path.dirname(output_path)):
                ceprint("ERROR: dnsgate is configured for 'mode = dnsmasq' in " +
...
def enable(config):
    if config.mode == 'dnsmasq':
        if not file_exists(OUTPUT_FILE_PATH):
            ceprint('ERROR: %s does not exist, ' +
        ...
        with open(OUTPUT_FILE_PATH, 'r') as fh:
            ...
            if 'mode: dnsmasq' not in file_content:
                ceprint('ERROR: %s was not generated in dnsmasq mode, ' +
        ...
        if not os.path.islink(symlink): # not a symlink
            if os.path.exists(symlink): # but exists
                ceprint("ERROR: " + symlink + " exists and is not a symlink. " +
                       "You need to manually delete it. Exiting.", level=LOG['ERROR'])
                quit(1)
        if is_broken_symlink(symlink): #hm, a broken symlink, ok, remove it
            ceprint("WARNING: removing broken symlink: %s", symlink, level=LOG['WARNING'])
            os.remove(symlink)
        ...
        ceprint("ERROR: enable is only available with --mode dnsmasq. Exiting.",
               level=LOG['ERROR'])
        quit(1)
...
def disable(config):
    if config.mode == 'dnsmasq':
        ...
        ceprint("ERROR: disable is only available with --mode dnsmasq. Exiting.",
               level=LOG['ERROR'])
        quit(1)
...
def blockall(config):
    if config.mode == 'dnsmasq':
        ...
        ceprint("ERROR: blockall is only available with --mode dnsmasq. Exiting.",
               level=LOG['ERROR'])
        quit(1)
...
def write_output_file(config, domains_combined):
    ...
    ceprint("Writing output file: %s in %s format", config.output, config.mode,
           level=LOG['INFO'])
...
def configure(sources, mode, block_at_psl, dest_ip, dnsmasq_config_file, output):
    if contains_whitespace(dnsmasq_config_file.name):
        ceprint("ERROR: --dnsmasq-config-file can not contain whitespace. Exiting.",
               level=LOG['ERROR'])
        quit(1)
...
def generate(config, no_cache, cache_expire):
    ...
    ceprint('Using output file: %s', config.output, level=LOG['INFO'])
    ...
    ceprint('%d validated remote blacklisted domains.',
           len(domains_combined_orig), level=LOG['INFO'])
    ...
    if config.block_at_psl and config.mode != 'hosts':
        ...
        ceprint("%d blacklisted domains left after stripping to PSL domains.",
               len(domains_combined), level=LOG['INFO'])

        if domains_whitelist:
            ceprint("Subtracting %d whitelisted domains.",
                   len(domains_whitelist), level=LOG['INFO'])
            domains_combined = domains_combined - domains_whitelist
            ceprint("%d blacklisted domains left after subtracting the whitelist.",
                   len(domains_combined), level=LOG['INFO'])
            ceprint('Iterating through the original %d whitelisted domains and ' +
        ...
        ceprint('Iterating through original %d blacklisted domains to re-add subdomains' +
            ...
            if orig_domain not in domains_whitelist: # if it's not in the whitelist
                if orig_domain not in domains_combined: # and it's not in the current blacklist
                    ...
                    if orig_domain_psl not in domains_combined: # if the psl is not already blocked
                        ceprint("Re-adding: %s", orig_domain, level=LOG['DEBUG'])
                        domains_combined.add(orig_domain) # add the full hostname to the blacklist

        ceprint('%d blacklisted domains after re-adding non-explicitly blacklisted subdomains',
               len(domains_combined), level=LOG['INFO'])

    elif config.block_at_psl and config.mode == 'hosts':
        ceprint("ERROR: --block-at-psl is not possible in hosts mode. Exiting.",
               level=-LOG['ERROR'])
        quit(1)
    ...
    ceprint("%d blacklisted domains after subtracting the %d whitelisted domains",
    ...
    try:
        ...
        ceprint('WARNING: %s is missing, only the default remote sources ' +
        ...
        if domains_blacklist: # ignore empty blacklist
            ceprint("Got %s domains from the CUSTOM_BLACKLIST: %s",
                   len(domains_blacklist), blacklist_file, level=LOG['DEBUG'])
            ceprint("Re-adding %d domains in the local blacklist %s to override the whitelist.",
                   len(domains_blacklist), CUSTOM_BLACKLIST, level=LOG['INFO'])
            domains_combined = domains_combined | domains_blacklist # union
            ceprint("%d blacklisted domains after re-adding the custom blacklist.",
                   len(domains_combined), level=LOG['INFO'])

    ceprint("Validating final domain blacklist.", level=LOG['DEBUG'])
    domains_combined = validate_domain_list(domains_combined)
    ceprint('%d validated blacklisted domains.', len(domains_combined), level=LOG['DEBUG'])

    prune_redundant_rules(domains_combined)
    ceprint('%d blacklisted domains after removing redundant rules.', len(domains_combined),
           level=LOG['INFO'])

    domains_combined = group_by_tld(domains_combined) # do last, returns sorted list
    ceprint('Final blacklisted domain count: %d', len(domains_combined), level=LOG['INFO'])

    if config.backup: # todo: unit test
        backup_file_if_exists(config.output)

    if not domains_combined:
        ceprint("The list of domains to block is empty, nothing to do, exiting.",
               level=LOG['INFO'])
        quit(1)
    ...
    for domain in domains_whitelist:
        ...
        if domain_tld in domains_combined:
            ceprint('WARNING: %s is listed in both %s and %s, '
...
if __name__ == '__main__':
    ...
    ceprint("Exiting without error.", level=LOG['DEBUG'])
    quit(0)
