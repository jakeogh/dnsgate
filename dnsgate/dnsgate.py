...
def set_verbose(ctx, param, verbose=False):
    if verbose:
        logger_quiet.logger.setLevel(LOG_LEVELS['DEBUG'])
    else:
...
DNSMASQ_CONFIG_SYMLINK   = DNSMASQ_CONFIG_INCLUDE_DIRECTORY + '/' + DEFAULT_OUTPUT_FILE_NAME
...
# Rules defined here override any conflicting rules in ''' + CUSTOM_WHITELIST + '''
#
# Examples:
...
# Rules defined here ARE OVERRIDDEN by any conflicting rules in ''' + CUSTOM_BLACKLIST + '''
#
# Examples:
...
def dnsgate(ctx, no_restart_dnsmasq, backup):
    ...
    if 'dnsgate configure' not in ' '.join(sys.argv):
        if 'dnsgate.py configure' not in ' '.join(sys.argv):
            ...
            if mode == 'dnsmasq':
                try:
                    dnsmasq_config_file = click.open_file(config['DEFAULT']['dnsmasq_config_file'], 'w', atomic=True, lazy=True)
                    dnsmasq_config_file.close()
                except KeyError:
                    eprint("ERROR: dnsgate is configured for 'mode = dnsmasq' in " + CONFIG_FILE +
                    " but dnsmasq_config_file is not set. run 'dnsmasq configure --help' to fix. Exiting.",
                    level=LOG_LEVELS['ERROR'])
                    quit(1)
    ...
    if config.mode == 'dnsmasq':
        ...
        if not is_unbroken_symlink_to_target(DEFAULT_OUTPUT_FILE, symlink):
            if os.path.islink(symlink): #hm, a broken symlink, ok, fix it
                eprint("WARNING: removing broken symlink: %s", dnsmasq, level=LOG_LEVELS['WARNING'])
                os.remove(symlink)
                symlink_relative(DEFAULT_OUTPUT, symlink)
        else: # not a symlink
            eprint("ERROR: " + symlink + " exists and is not a symlink. You need to manually delete it. Exiting.", level=LOG_LEVELS['ERROR'])
            quit(1)
...
def disable(config):
    if config.mode == 'dnsmasq':
        comment_out_line_in_file(config.dnsmasq_config_file, generate_dnsmasq_config_file_line())
        config.dnsmasq_config_file.close()
        restart_dnsmasq_service()
        if len(sys.argv) > 2:
...
def configure(mode, block_at_psl, dest_ip, dnsmasq_config_file):
    if contains_whitespace(dnsmasq_config_file.name):
        eprint("ERROR: --dnsmasq-config-file can not contain whitespace. Exiting.", level=LOG_LEVELS['ERROR'])
        quit(1)
...
def generate(config, sources, no_cache, cache_expire, output):
    ...
    try:
        ...
        eprint('WARNING: %s is missing, only the default remote sources will be used. Run "dnsgate configure --help" to fix.',
            CUSTOM_WHITELIST, level=LOG_LEVELS['WARNING'])
    else:
        if domains_whitelist:
            eprint("%d domains from %s", len(domains_whitelist), str(CUSTOM_WHITELIST), level=LOG_LEVELS['DEBUG'])
    ...
    if not domains_whitelist:
        if config.block_at_psl:
            eprint('WARNING: block_at_psl is enabled in ' + CONFIG_FILE + ' and 0 domains were obtained from %s. ' +
    ...
    eprint("Validating final domain blacklist.", level=LOG_LEVELS['DEBUG'])
    ...
    if not domains_combined:
        eprint("The list of domains to block is empty, nothing to do, exiting.", level=LOG_LEVELS['INFO'])
        quit(1)
